        ...
        public static object ListarCartillaAtencion()
        {
            using (var db = new EFData.PET_DBEntities())
            {
                ...
                var cartilla = (from cartillas in db.Cartilla_Atencion
                             ...
                             join clientes in db.Clientes on cartillas.id_cliente equals clientes.id_cliente
                             join pacientes in db.Pacientes on cartillas.id_paciente equals pacientes.id_paciente
                             join empleados in db.Empleadoes on cartillas.id_empleado equals empleados.id_empleado
                             select new
                             {
                                 ...
                                 NOMEMP = empleados.nombres.ToUpper() + " " + empleados.apellido_paterno.ToUpper() + " " + empleados.apellido_materno.ToUpper(),
        ...
        public static object ListaCartillaAtencion(string sidx, string sord, int page, int rows, bool _search)
        {
            using (var db = new EFData.PET_DBEntities())
            {
                var cartilla = (from ca in db.Cartilla_Atencion
                                join c in db.Clientes on ca.id_cliente equals c.id_cliente
                                join e in db.Empleadoes on ca.id_cliente equals e.id_empleado
                                join p in db.Pacientes on ca.id_paciente equals p.id_paciente
                                select new
                                {
                                    NUMCOM = ca.numero_comprobante,
                                    NOMCLI = c.nombres + " " + c.apellido_paterno + " " + c.apellido_materno,
                                    NOMPAC = p.nombre,
                                    NOMEMP = e.nombres + " " + e.apellido_paterno + " " + e.apellido_materno,
                                    CODIGO = ca.codigo,
                ...
                using (var db = new EFData.PET_DBEntities())
                {
                    ...
                    var cartillaAtencion = 
                        new Pet.Data.EF5.EFData.Cartilla_Atencion {id_cartilla_atencion = cartilla.id_cartilla_atencion,
                                                                   ...
                                                                   id_empleado = cartilla.id_empleado,
